//
//  TPOCRScanViewController.swift
//  VegaFintech
//
//  Created by tran dinh thong on 8/30/21.
...
public var TPFrontImage: UIImage? //= UIImage()

public var TPBackImage: UIImage?

extension TPOCRScanViewController: CaptureManagerDelegate {
    func processCapturedImage(image: UIImage) {
        self.image = image
    }
}

class TPOCRScanViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
...
//    @IBOutlet weak var sideLabelCenterYConstraint: NSLayoutConstraint!
    ...
    public var frontImage: UIImage? //= UIImage()
...
//            posY = self.previewLayer.frame.height / 2 - 100
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if self.isTapped {
            ...
            DispatchQueue.main.async {
                //self.showSimpleAlert(title: "123", ms: "123")
            }
            ...
            DispatchQueue.main.async {
                let destinationVC = TPPreviewViewController()
                destinationVC.previewImage = finalImg
                destinationVC.isFront = self.isFront
                destinationVC.delegate = self
                //self.navigationController?.presentViewController(destinationVC)
                self.presentViewController(destinationVC, transitionStyle: .coverVertical, modalPresentationStyle: .fullScreen )
...
extension TPOCRScanViewController: PreviewControllerDelegate {
    func userDidConfirm(image: UIImage, isFront: Bool) {
        ...
        if !isFront {
            ...
            //self.sideLabel.text = "Mặt trc"
            if frontImage != nil && backImage != nil {
                let destinationVC = TPResultOCRViewController()
                destinationVC.imageFront = frontImage
                destinationVC.imageBack = backImage
                //TPFrontImage = frontImage
                //TPBackImage = backImage
                    //TPFaceMatchGuideViewController()
