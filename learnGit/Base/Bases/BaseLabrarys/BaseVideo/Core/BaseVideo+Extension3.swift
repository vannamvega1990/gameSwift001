...
extension BaseVideo {
    ...
    func makeBirthdayCard(fromVideoAt videoURL: URL, forName name: String, onComplete: @escaping (URL?) -> Void) {
        ...
        //videoLayer.contents = UIImage(named: "background")?.cgImage
    ...
    private func add(text: String, to layer: CALayer, videoSize: CGSize) {
      let attributedText = NSAttributedString(
        string: text,
        attributes: [
          .font: UIFont(name: "ArialRoundedMTBold", size: 46) as Any,
      ...
      textLayer.frame = CGRect(
        x: -videoSize.width/2 + 176,
        //y: videoSize.height * 0.66,
        y: videoSize.height * 0.72,
    ...
    private func addConfetti(to layer: CALayer) {
        ...
        var colors: [UIColor] = [.systemGreen, .systemRed, .systemBlue, .systemPink, .systemOrange, .systemPurple, .systemYellow]
        colors = colors.map({ (color) -> UIColor in
            color.withAlphaComponent(0.46)
        })
        let cells: [CAEmitterCell] = (0...6).map { _ in
